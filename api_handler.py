...
def print_bytey():
    ...
    Prints Bytey the ascii-dog to the terminal
       ...
       '\~ o/` ,,
        ...
        /`-'\$ 
       (_/-\_) 
    Credit from: https://www.asciiart.eu/animals/dogs (Linda Ball)
    """
    print(',\'.-.\'.')
    print('\'\\~ o/` ,,')
    print(' { @ } f')
...
def get_scrobs_from_page(j_recenttracks):
    ...
    for track in j_tracks:
        ...
        if date != None:
            ...
            scrobbled_data_path = get_path('scrobbled_data')
...
def init_user_info():
    ...
    # write the current username to a special file so that the Java program
    # knows which user text files to look for
    with open('current_user.txt', 'w') as f:
        f.write(username)
    # time to write to our user_info text file
    user_info_path = get_path('user_info')
...
def get_path(directory) -> str:
    """
    Generates the absolute path for a newly created file
    """
    return os.getcwd() + '/' + directory + '/' + username + '.txt'
